...
class Predictor(nn.Module):
    def __init__(self, input_dim, fixed_feat=7, num_blocks=3, dropout_rate=0.4, feature_drop_rate=0.04):
        ...
        self.stage1 = nn.ModuleList([
            AdvancedResidualBlock(hidden_dim, dropout_rate) for _ in range(num_blocks)
        ])
        ...
        self.stage2 = nn.ModuleList([
            AdvancedResidualBlock(mid_dim, dropout_rate / 2) for _ in range(num_blocks - 1)
        ])
